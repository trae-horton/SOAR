...
def File_Filter(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None):
    ...
    matched_artifacts_3, matched_results_3 = phantom.condition(
        ...
        action_results=results,
        conditions=[
            ["file_reputation_1:action_result.data.*.positives", "==", "\"NULL\""],
        ],
        name="File_Filter:condition_3")

    # call connected blocks if filtered artifacts or results
    if matched_artifacts_3 or matched_results_3:
        prompt_5(action=action, success=success, container=container, results=results, handle=handle, filtered_artifacts=matched_artifacts_3, filtered_results=matched_results_3)
...
def prompt_5(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None):
    ...
    # set user and message variables for phantom.prompt call
    user = "admin"
    message = """No result in VirusTotal
{0}"""
    ...
    phantom.prompt2(container=container, user=user, message=message, respond_in_mins=30, name="prompt_5", parameters=parameters, response_types=response_types)
